...
- Use \`github.com/org/\` pattern to find repos in an org`,
	timeoutMs: 15000,
	schema: z.object({
		query: z.string().optional().describe("Filter repositories by name pattern (optional)"),
		maxResults: z
			.number()
			.int()
			.min(1)
	...
	execute: async ({ query, maxResults }) => {
		...
		if (repos.length === 0) {
			return query ? `No repositories found matching: "${query}"` : "No repositories found.";
		}
		...
		const moreNote = hasMore ? ` (showing ${repos.length} of ${data.repositories.totalCount})` : "";
		...
		if (hasMore) {
			lines.push(`Note: More repositories available. Increase maxResults or refine your query.`);
		}
